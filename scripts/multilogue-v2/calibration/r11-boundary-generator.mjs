...
export const R11_BOUNDARY_CONFIGS = Object.freeze([
  { enabled: false, name: 'r10_control' },
  ...[20, 30].flatMap((smoothingMs) => [20, 30].flatMap((onsetStableRunMs) => [20, 30].map((offsetStableRunMs) => ({
    enabled: true,
    name: `smooth${smoothingMs}_on${onsetStableRunMs}_off${offsetStableRunMs}`,
    onsetRadiusMs: 250,
    offsetRadiusMs: 250,
    smoothingMs,
    hysteresisDb: 3,
...
export function generateR11BoundaryCandidates({
  outputDir,
  ...
  configs = R11_BOUNDARY_CONFIGS,
} = {}) {
  if (!outputDir) throw new Error('outputDir is required');
  if (existsSync(outputDir)) throw new Error(`R11 generator output already exists: ${outputDir}`);
  if (!Array.isArray(configs) || configs.length < 2) throw new Error('R11 requires control and boundary candidates');
  ...
  for (const boundaryConfig of configs) {
    const candidateId = `r11-${hashText(stableJson(boundaryConfig)).slice(0, 12)}`;
    ...
    const refinement = refineSpeakerLocalPhraseBoundaries(built.input.stage1Evidence, support, boundaryConfig);
    const semanticDigestAfter = semanticDigest(refinement.events);
    if (semanticDigestAfter !== semanticDigestBefore) throw new Error('boundary refinement changed semantic identity');
    ...
    if (!tier5.pass) throw new Error(`R11 Tier5 consistency failed: ${tier5.errors.join('; ')}`);
    ...
    writeJson(evidenceFile, {
      contract_version: 'r11-boundary-runtime-evidence-v1',
      candidate_id: candidateId,
      boundary_config: boundaryConfig,
      boundary_refinement: refinement,
    ...
    writeJson(manifestFile, {
      contract_version: 'r11-boundary-generator-manifest-v1',
      candidate_id: candidateId,
    ...
    writeJson(path.join(candidateDir, 'artifact-hashes.json'), {
      contract_version: 'r11-boundary-candidate-hashes-v1', candidate_id: candidateId,
      files: hashes, aggregate_sha256: aggregate,
    });
    index.push({ candidate_id: candidateId, candidate_dir: `candidates/${candidateId}`, boundary_config: boundaryConfig, aggregate_sha256: aggregate });
  }
  const indexFile = path.join(outputDir, 'candidate-index.json');
  writeJson(indexFile, {
    contract_version: 'r11-boundary-candidate-index-v1', candidate_count: index.length,
    expected_candidate_count: configs.length, runtime_gold_access: false,
...
function assertR10Config(config) {
  if (Number(config.adapter?.acousticThresholdMarginDb) !== 5) throw new Error('R11 requires frozen R10 margin 5');
  if (config.adapter?.residualIdentityPolicy !== 'bounded_margin') throw new Error('R11 requires frozen R10 identity policy');
  if (config.backchannel?.mode !== 'explicit_plus_acoustic') throw new Error('R11 requires frozen R10 backchannel mode');
}
...
function parseArgs(argv) {
  if (argv.includes('--gold')) throw new Error('R11 generator rejects Gold inputs');
  const options = {};
  for (let index = 2; index < argv.length; index += 2) {
    ...
    else throw new Error(`unknown R11 generator argument: ${key}`);
  }
  return options;
}

if (process.argv[1] && import.meta.url === pathToFileURL(path.resolve(process.argv[1])).href) {
  const result = generateR11BoundaryCandidates(parseArgs(process.argv));
