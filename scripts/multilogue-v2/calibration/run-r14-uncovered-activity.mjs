...
const GENERATOR = path.join(ROOT, 'scripts', 'multilogue-v2', 'calibration', 'r14-uncovered-activity-generator.mjs');
const SCORER = path.join(ROOT, 'scripts', 'multilogue-v2', 'calibration', 'r14-uncovered-activity-scorer.mjs');
const GOLD = path.join(ROOT, 'outputs', 'multilogue-v2-poc', `${RECORDING_ID}_P025_corrected_6tier.TextGrid`);

export function runR14UncoveredActivity({ outputDir, goldFile = GOLD } = {}) {
  if (!outputDir) throw new Error('outputDir is required');
  if (existsSync(outputDir)) throw new Error(`R14 output already exists: ${outputDir}`);
  mkdirSync(outputDir, { recursive: true });
  const generated = parseLastJson(runChild([
    GENERATOR, '--output-dir', path.join(outputDir, 'candidates'),
  ], 'R14 generator').stdout);
  ...
  const result = {
    contract_version: 'r14-uncovered-activity-final-v1',
    status: score.formal_gate.pass ? 'R14_FORMAL_GATE_PASS' : 'R14_FORMAL_GATE_FAIL',
    ...
    selected_candidate: {
      ...
      uncovered_activity_config: score.winner.uncovered_activity_config,
      uncovered_activity_stats: score.winner.uncovered_activity_stats,
    ...
    recommendation: score.formal_gate.pass
      ? 'R14 may proceed to unseen multilogue validation.'
      : 'Do not promote R14; retain the best prior non-regressing baseline.',
  };
...
function runChild(args, label) {
  const result = spawnSync(process.execPath, args, {
    cwd: ROOT, encoding: 'utf8', env: { ...process.env, MWU_NETWORK_DISABLED: '1' }, maxBuffer: 300 * 1024 * 1024,
  });
...
function parseArgs(argv) {
  ...
  for (let index = 2; index < argv.length; index += 2) {
    ...
    else throw new Error(`unknown R14 runner argument: ${key}`);
  }
  return options;
}
if (process.argv[1] && import.meta.url === pathToFileURL(path.resolve(process.argv[1])).href) {
  const result = runR14UncoveredActivity(parseArgs(process.argv));
