...
export const R12_TARGETS = Object.freeze({
...
const CONTROL_GUARD_KEYS = Object.freeze(Object.keys(R12_TARGETS).filter((key) => key !== 'boundary_f1_100ms'));

export function scoreR12InternalGapCandidates({ candidateRoot, expectedIndexSha256, goldFile, outputFile }) {
  const indexFile = path.join(candidateRoot, 'candidate-index.json');
  if (fileRecord(indexFile).sha256 !== expectedIndexSha256) throw new Error('R12 candidate index SHA mismatch');
  const index = readJson(indexFile);
  if (index.runtime_gold_access !== false || index.network_used !== false
    || index.candidate_count !== index.expected_candidate_count) throw new Error('R12 index contract invalid');
  ...
  if (JSON.stringify(actualNames) !== JSON.stringify(indexedNames)) throw new Error('R12 candidate set mismatch');
  const rows = index.candidates.map((candidate) => scoreCandidate(candidateRoot, candidate, goldFile));
  const control = rows.find((row) => row.internal_gap_config.enabled === false);
  if (!control) throw new Error('R12 control candidate missing');
  const eligible = rows.filter((row) => controlRelativeGate(row.metrics, control.metrics));
  const ranked = (eligible.length ? eligible : [control]).sort(compareRows);
  const winner = ranked[0];
  const report = {
    contract_version: 'r12-internal-gap-score-v1',
    candidate_index_sha256: expectedIndexSha256,
    candidate_count: rows.length,
    control,
    winner,
    control_relative_gate: controlRelativeGateReport(winner.metrics, control.metrics),
    formal_gate: formalGate(winner.metrics),
    candidates: rows.sort((left, right) => left.candidate_id.localeCompare(right.candidate_id)),
    selection_policy: 'no regression on seven R10 semantic KPIs, then maximize boundary_f1_100ms',
...
function scoreCandidate(root, candidate, goldFile) {
  ...
  if (manifest.runtime_gold_access !== false || manifest.network_used !== false
    || manifest.room_mix_boundary_crossing !== false || manifest.outer_boundaries_moved !== false) {
    throw new Error('R12 candidate runtime isolation violation');
  }
  ...
  return {
    ...
    internal_gap_config: candidate.internal_gap_config,
    internal_gap_stats: evidence.internal_gap_refinement.stats,
...
function controlRelativeGate(metrics, control) {
  return CONTROL_GUARD_KEYS.every((key) => Number(metrics[key]) + 1e-9 >= Number(control[key]));
}

function controlRelativeGateReport(metrics, control) {
  const results = Object.fromEntries(CONTROL_GUARD_KEYS.map((key) => [key, {
    control: Number(control[key]),
...
function compareRows(left, right) {
  return Number(right.metrics.boundary_f1_100ms) - Number(left.metrics.boundary_f1_100ms)
    ...
    || left.internal_gap_stats.inserted_gap_count - right.internal_gap_stats.inserted_gap_count
    || left.candidate_id.localeCompare(right.candidate_id);
}

function formalGate(metrics) {
  const results = Object.fromEntries(Object.entries(R12_TARGETS).map(([key, target]) => [key, {
...
function verifyHashes(dir, aggregateExpected) {
  ...
  for (const [name, expected] of Object.entries(document.files)) {
    ...
    if (actual.sha256 !== expected.sha256 || actual.bytes !== expected.bytes) throw new Error(`R12 hash mismatch: ${name}`);
  }
  if (hashText(stableJson(document.files)) !== aggregateExpected) throw new Error('R12 aggregate hash mismatch');
}
...
function parseArgs(argv) {
  ...
  for (let index = 2; index < argv.length; index += 2) {
    ...
    else throw new Error(`unknown R12 scorer argument: ${key}`);
  }
  return options;
}
if (process.argv[1] && import.meta.url === pathToFileURL(path.resolve(process.argv[1])).href) {
  const report = scoreR12InternalGapCandidates(parseArgs(process.argv));
